    ...
    >
      ...
      <div className="relative w-full max-w-xl h-56 md:h-64 flex items-center justify-center mb-10">
        <motion.div
          ...
          className="relative z-10 w-20 h-20 bg-[#009DFF]/10 border-2 border-[#009DFF]/50 rounded-full flex items-center justify-center mt-16"
        ...
        <svg className="absolute inset-0 w-full h-full z-0" fill="none" viewBox="0 0 480 240">
          ...
          {paths.map((path, i) => (
            <React.Fragment key={i}>
              ...
              <motion.circle id={`dot-${i}`} r="4" fill="#64E1FF" style={{ filter: 'blur(2px)', offsetPath: `path('${path}')` }} />
            </React.Fragment>
          ))}
        </svg>
